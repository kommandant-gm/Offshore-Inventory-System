...
<div class="header"><img class="logo" src="{{ $logoPath }}"><h1>Miri Inventory Registration Form</h1><p class="sub">{{ $equipment->inventory_type === 'cargo' ? 'Cargo' : 'Machinery' }} record · {{ $equipment->tag_no ?: 'No tag number' }}</p></div>
...
@if($equipment->inventory_type !== 'cargo')
...
@if($equipment->inventory_type === 'cargo')
<h2>Cargo specifications</h2><table><tr><td class="label">Dimensions / Model</td><td>{{ $equipment->size_model ?: '-' }}</td><td class="label">Tonnage</td><td>{{ $equipment->size_ton ?: '-' }}</td></tr><tr><td class="label">Length</td><td>{{ $equipment->size_length ?: '-' }}</td><td class="label">Quantity</td><td>{{ $equipment->quantity ?? 'Not recorded' }} {{ $equipment->quantity !== null ? $equipment->unit : '' }}</td></tr></table>
@endif
