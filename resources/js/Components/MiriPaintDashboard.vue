...
import PaintQuantityCharts from '@/Components/PaintQuantityCharts.vue';
const props = defineProps({ dashboard: Object, canEdit: Boolean });
const cards = [['total','Paint records',''], ['expired','Expired','expired'], ['due_30_days','Expire in 30 days','due_30_days'], ['unconfirmed','Unconfirmed dates','unconfirmed'], ['duplicates','Possible repeat rows','duplicates'], ['review','Needs review','review']];
const dateGroups = computed(() => [
    { key: 'expired', label: 'Expired', color: '#ef4444', filter: 'expired' },
    { key: 'due_30_days', label: 'Expire in 30 days', color: '#f59e0b', filter: 'due_30_days' },
    { key: 'beyond_30_days', label: 'Expire after 30 days', color: '#10b981' },
...
<template>
    <section class="space-y-6">
        <header class="rounded-[28px] bg-gradient-to-r from-[#075442] to-[#218487] p-7 text-white shadow-lg sm:p-9">
            ...
            <p class="mt-3 max-w-2xl text-sm leading-6 text-teal-100">Paint stock in litres, locations and confirmed expiry dates in one overview.</p>
        ...
        <p class="rounded-xl bg-amber-50 p-4 text-sm text-amber-900">Unconfirmed single dates never trigger best-before alerts. Counts represent source records, not unique products. New Paint COGs update closing stock; monthly opening balances carry forward. Cans and litres remain separate.</p>
        ...
        <div class="grid gap-5 lg:grid-cols-2">
            <section class="rounded-3xl border border-[#d8e7d4] bg-white p-6">
                <p class="text-xs font-bold tracking-widest text-indigo-500">DATE COVERAGE</p><h2 class="mt-2 text-lg font-bold text-[#234222]">Expiry Overview</h2>
            ...
            <PaintQuantityCharts :quantities="dashboard.quantities" />
        </div>
