...
<template>
  ...
  <AuthenticatedLayout>
    <section class="space-y-6">
      ...
      <section class="rounded-[1.7rem] border border-[#d8e7d4] bg-white p-6 shadow-sm"><div class="flex flex-wrap items-center justify-between gap-3"><h2 class="text-xl font-bold text-[#234222]">Message content</h2><span v-if="log.reconstructed" class="rounded-full bg-[#fff1bd] px-3 py-1 text-xs font-bold text-[#914400]">Reconstructed from saved records</span></div><pre class="mt-4 whitespace-pre-wrap rounded-xl bg-[#f7fbf5] p-5 text-sm leading-7 text-[#31415b]">{{ log.body || 'Content was not captured for this older activity record.' }}</pre></section>
