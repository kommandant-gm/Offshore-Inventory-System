...
<header class="flex flex-wrap justify-between gap-4 rounded-3xl border bg-white p-6"><div><h1 class="text-2xl font-bold text-[#234222]">{{ cog.display_cog_no }}</h1><p class="mt-2 text-sm text-slate-500">{{ cog.movement_type }} · {{ cog.status }} · Document only</p></div><div class="flex gap-2"><a :href="route('miri-cogs.pdf',cog.id)" class="btn bg-[#234222] text-white">Download PDF</a><Link :href="route('miri-cogs.index')" class="btn">Back to register</Link></div></header>
