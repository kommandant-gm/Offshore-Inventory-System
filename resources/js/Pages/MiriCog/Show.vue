...
const stockConfirmation = useForm({ confirmed: false });
const confirmStock = () => stockConfirmation.post(route('miri-cogs.confirm-stock', props.cog.id), { preserveScroll: true });
...
<header class="flex flex-wrap justify-between gap-4 rounded-3xl border bg-white p-6"><div><h1 class="text-2xl font-bold text-[#234222]">{{ cog.display_cog_no }}</h1><p class="mt-2 text-sm text-slate-500">{{ cog.movement_type }} · {{ cog.status }}</p></div><div class="flex flex-wrap gap-2"><Link v-if="canEdit && cog.status === 'draft' && !cog.signature && !cog.construction_stock_confirmed_at" :href="route('miri-cogs.edit', cog.id)" class="btn text-green-800">Edit</Link><a :href="route('miri-cogs.pdf',cog.id)" class="btn bg-[#234222] text-white">Download PDF</a><Link :href="route('miri-cogs.index')" class="btn">Back to register</Link></div></header>
<section v-if="cog.construction_stock_workflow" class="rounded-2xl border bg-white p-5">
    <h2 class="font-bold text-[#234222]">TEC, Garnet &amp; PPE stock confirmation</h2>
    <p v-if="cog.construction_stock_confirmed_at" class="mt-2 text-sm text-green-800">Stock posted on {{ cog.construction_stock_confirmed_at }}. Repeated confirmation will not post again.</p>
    <form v-else-if="canEdit && ['draft', 'signed'].includes(cog.status)" class="mt-3 space-y-3" @submit.prevent="confirmStock">
        <p class="text-sm">Confirm only after the physical issue, receipt or transfer has taken place. This updates all TEC, Garnet &amp; PPE stock lines together. A confirmed note cannot be cancelled as an unfulfilled draft.</p>
        <ul class="list-inside list-disc text-sm"><li v-for="line in cog.items.filter(item => item.item_type === 'Construction')" :key="line.id">#{{ line.item_id }} {{ line.description }} — {{ line.quantity }} {{ line.unit }}<span v-if="line.construction_destination_id"> → destination record #{{ line.construction_destination_id }}</span></li></ul>
        <label class="flex gap-2 text-sm"><input v-model="stockConfirmation.confirmed" required type="checkbox" />I have verified the quantities, units and locations and confirm this movement.</label>
        <p v-for="(error, key) in stockConfirmation.errors" :key="key" class="text-sm text-red-700" role="alert">{{ error }}</p>
        <button class="btn bg-[#234222] text-white" :disabled="!stockConfirmation.confirmed || stockConfirmation.processing">Confirm stock movement</button>
    </form>
    <p v-else class="mt-2 text-sm text-slate-600">Stock has not been posted for this note.</p>
</section>
<form v-if="canEdit && cog.status === 'draft' && (['Issue out','Transfer','Return to supplier'].includes(cog.movement_type) || (cog.construction_stock_workflow && cog.items.every(line => line.item_type === 'Construction')))" class="rounded-xl border bg-white p-4" @submit.prevent="cancel">
