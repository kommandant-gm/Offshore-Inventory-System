...
const props = defineProps({ summary: Object, categories: Array, locations: Array, recent: Array });
const total = computed(() => Number(props.summary?.total ?? 0));
const statuses = computed(() => [
    { label: 'In Use', value: Number(props.summary?.in_use ?? 0), color: '#2563eb' },
    { label: 'Standby', value: Number(props.summary?.standby ?? 0), color: '#10b981' },
    { label: 'Under Repair', value: Number(props.summary?.under_repair ?? 0), color: '#f59e0b' },
    { label: 'Damaged', value: Number(props.summary?.damaged ?? 0), color: '#ef4444' },
]);
        ...
        <section class="dashboard-shell space-y-6">
            <header class="relative isolate overflow-hidden rounded-[1.75rem] bg-[linear-gradient(120deg,#064e3b_0%,#0f766e_58%,#115e59_100%)] px-5 py-6 text-white shadow-[0_24px_70px_rgba(6,78,59,.22)] sm:px-8 sm:py-8 lg:px-10">
                ...
                <div class="relative grid items-end gap-8 lg:grid-cols-[minmax(0,1fr),22rem]">
                    <div><p class="text-xs font-semibold uppercase tracking-[.24em] text-emerald-200">Miri Inventory</p><h1 class="mt-3 text-3xl font-bold tracking-[-.02em] sm:text-4xl lg:text-[2.75rem]">Inventory Dashboard</h1><p class="mt-3 max-w-2xl text-sm leading-6 text-white/65 sm:text-base">Monitor equipment, categories, locations, and operational status from one responsive workspace.</p><div class="mt-6 flex flex-wrap gap-3"><Link :href="route('major-equipment.index')" class="inline-flex items-center rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-emerald-900 shadow-sm transition hover:-translate-y-0.5 hover:bg-emerald-50">View inventory register</Link><Link :href="route('major-equipment.import')" class="inline-flex items-center rounded-xl border border-white/20 bg-white/10 px-4 py-2.5 text-sm font-bold text-white backdrop-blur transition hover:-translate-y-0.5 hover:bg-white/15">+ Import CSV</Link></div></div>
                    <div class="grid grid-cols-2 gap-3 rounded-2xl border border-white/10 bg-white/[.07] p-4 backdrop-blur-sm"><div class="rounded-xl bg-black/10 p-4"><p class="text-[10px] font-semibold uppercase tracking-[.16em] text-white/50">In use</p><p class="mt-2 text-2xl font-bold text-cyan-200 sm:text-3xl">{{ percent(summary.in_use) }}%</p><p class="mt-1 text-xs text-white/55">{{ summary.in_use }} records</p></div><div class="rounded-xl bg-black/10 p-4"><p class="text-[10px] font-semibold uppercase tracking-[.16em] text-white/50">Standby</p><p class="mt-2 text-2xl font-bold text-emerald-300 sm:text-3xl">{{ percent(summary.standby) }}%</p><p class="mt-1 text-xs text-white/55">{{ summary.standby }} records</p></div></div>
                </div>
            </header>

            <div class="grid grid-cols-2 gap-3 lg:grid-cols-4 lg:gap-4">
                <div v-for="card in [{label:'Total Records',value:summary.total,accent:'bg-[#234222]',icon:'T'},{label:'In Use',value:summary.in_use,accent:'bg-blue-500',icon:'I'},{label:'Standby',value:summary.standby,accent:'bg-emerald-500',icon:'S'},{label:'Under Repair',value:summary.under_repair,accent:'bg-amber-500',icon:'U'}]" :key="card.label" class="relative overflow-hidden rounded-2xl border border-[#d8e7d4] bg-white p-4 shadow-[0_8px_28px_rgba(39,89,45,.06)] sm:p-5"><div class="absolute inset-x-0 top-0 h-1" :class="card.accent"/><div class="flex items-start justify-between gap-3"><div><p class="text-[10px] font-extrabold uppercase tracking-[.16em] text-[#70836e] sm:text-xs">{{ card.label }}</p><p class="mt-2 text-2xl font-black text-[#173a21] sm:text-3xl">{{ card.value }}</p><p class="mt-1 text-[11px] font-semibold text-[#8a9a88]">{{ percent(card.value) }}% of records</p></div><span class="flex h-9 w-9 items-center justify-center rounded-xl bg-[#e7f3e3] text-sm font-black text-[#234222]">{{ card.icon }}</span></div></div>
            </div>

            <div class="grid grid-cols-1 gap-4 lg:grid-cols-12 lg:gap-5">
                <article class="rounded-[1.5rem] border border-[#d9e8d5] bg-white p-5 shadow-[0_12px_35px_rgba(39,89,45,.07)] sm:p-6 lg:col-span-5"><div class="flex items-start justify-between gap-4"><div><p class="text-[10px] font-extrabold uppercase tracking-[.22em] text-blue-500">Equipment allocation</p><h2 class="mt-1 text-xl font-black tracking-tight text-slate-800">Status overview</h2></div><span class="text-right text-xs text-slate-400"><strong class="block text-lg text-slate-700">{{ total }}</strong>records</span></div><div class="mt-7 grid items-center gap-7 sm:grid-cols-[12rem,minmax(0,1fr)]"><div class="relative mx-auto h-44 w-44 rounded-full p-3 shadow-inner sm:h-48 sm:w-48" :style="{ background: pie }"><div class="flex h-full w-full flex-col items-center justify-center rounded-full bg-white shadow-[inset_0_0_0_1px_#edf4ea]"><span class="text-3xl font-black text-[#173a21]">{{ total }}</span><span class="mt-1 text-xs font-semibold text-[#7b8f78]">Total records</span></div></div><div class="space-y-2"><div v-for="item in statuses" :key="item.label" class="flex items-center gap-3 rounded-xl px-3 py-2.5"><span class="h-2.5 w-2.5 shrink-0 rounded-full ring-4 ring-[#edf5ea]" :style="{ backgroundColor: item.color }"/><span class="min-w-0 flex-1 truncate text-sm font-semibold text-[#5c715a]">{{ item.label }}</span><span class="text-right"><strong class="block text-sm font-black text-[#173a21]">{{ item.value }}</strong><small class="text-[10px] font-bold text-[#93a391]">{{ percent(item.value) }}%</small></span></div></div></div></article>
                <article class="rounded-[1.5rem] border border-[#d9e8d5] bg-white p-5 shadow-[0_12px_35px_rgba(39,89,45,.07)] sm:p-6 lg:col-span-7"><div class="flex items-start justify-between gap-4"><div><p class="text-[10px] font-extrabold uppercase tracking-[.22em] text-indigo-500">Inventory mix</p><h2 class="mt-1 text-xl font-black tracking-tight text-slate-800">Records by category</h2></div><p class="text-right text-xs text-slate-400"><strong class="block text-lg text-slate-700">{{ categories.length }}</strong>categories</p></div><div class="mt-7 space-y-5"><div v-for="item in categories" :key="item.category" class="rounded-xl p-2"><div class="mb-2 flex items-end justify-between gap-4"><span class="truncate text-sm font-bold text-[#50694e]">{{ item.category || 'Uncategorised' }}</span><span class="shrink-0 text-sm"><strong class="text-[#173a21]">{{ item.total }}</strong><small class="ml-1 text-[#91a08f]">({{ percent(item.total) }}%)</small></span></div><div class="h-3 overflow-hidden rounded-full bg-slate-100"><div class="h-full rounded-full bg-[linear-gradient(90deg,#2563eb,#8b5cf6)]" :style="{ width: `${(item.total / maximum(categories)) * 100}%` }"/></div></div><p v-if="categories.length === 0" class="text-sm text-slate-500">No records imported yet.</p></div></article>
