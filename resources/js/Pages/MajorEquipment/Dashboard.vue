...
const props = defineProps({ companyFilter: String, paintDashboard: Object, canEditPaint: Boolean, summary: Object, categories: Array, recent: Array, expiry: Object, expiring: Array, rentalDashboard: Object, constructionDashboard: Object, canEditConstruction: Boolean, activeDashboard: String, inventoryType: String, typeCounts: Object, statusBreakdown: Array, quality: Object, quantityRecorded: Number });
const selectedCompany = ref(props.companyFilter || '');
watch(() => props.companyFilter, value => selectedCompany.value = value || '');
const applyCompany = value => router.get(route('major-equipment.dashboard'), { view: activeDashboard.value, inventory_type: props.inventoryType || 'all', company: value }, { preserveScroll: true });
...
const primaryStatuses = ['In Use', 'Standby', 'Under Repair'];
const statusLabel = (label) => ({ 'In Use': 'Usage / In Use', 'Standby': 'Standby / Ready to Use' })[label] || label;
const normalizeStatus = (label) => ({ 'IN USE': 'In Use', 'STANDBY': 'Standby', 'UNDER REPAIR': 'Under Repair', 'PENDING REPAIR': 'Under Repair', 'DAMAGED': 'Damaged', 'NOT RECORDED': 'Not recorded' })[label.trim().toUpperCase()] || label.trim();
const statusColors = computed(() => {
    const palette = { 'In Use': '#2563eb', 'Standby': '#10b981', 'Under Repair': '#f59e0b', 'Damaged': '#ef4444', 'Not recorded': '#94a3b8' };
    const extras = [...new Set((props.statusBreakdown ?? []).map(item => normalizeStatus(item.label)))].filter(label => !palette[label]).sort();
    extras.forEach((label, index) => palette[label] = colors.slice(3)[index % (colors.length - 3)]);
    return palette;
});
const groupStatuses = (rows = []) => {
    const counts = new Map(primaryStatuses.map(label => [label, 0]));
    rows.forEach(item => {
        const label = normalizeStatus(item.label);
        counts.set(label, (counts.get(label) || 0) + Number(item.value));
    });
    return [...counts].map(([label, value]) => ({ label, value, color: statusColors.value[label] }))
        .sort((a, b) => {
            const rank = label => primaryStatuses.includes(label) ? primaryStatuses.indexOf(label) : 3;
            return rank(a.label) - rank(b.label) || a.label.localeCompare(b.label);
        });
};
const statuses = computed(() => groupStatuses(props.statusBreakdown));
const selectedSubcategory = ref('');
const selectedCategory = computed(() => (props.categories ?? []).find(item => item.category === selectedSubcategory.value));
const overviewTotal = computed(() => selectedCategory.value ? Number(selectedCategory.value.total) : total.value);
const overviewStatuses = computed(() => selectedCategory.value ? groupStatuses(selectedCategory.value.statuses) : statuses.value);
const statusPageSize = 6;
const statusPage = ref(1);
const statusPageCount = computed(() => Math.max(1, Math.ceil(overviewStatuses.value.length / statusPageSize)));
const statusStart = computed(() => (statusPage.value - 1) * statusPageSize);
const visibleStatuses = computed(() => overviewStatuses.value.slice(statusStart.value, statusStart.value + statusPageSize));
watch([() => props.inventoryType, () => props.statusBreakdown, selectedSubcategory], () => {
    statusPage.value = 1;
});
...
const categoryPageCount = computed(() => Math.max(1, Math.ceil((props.categories?.length ?? 0) / categoryPageSize)));
const categoryStart = computed(() => (categoryPage.value - 1) * categoryPageSize);
const visibleCategories = computed(() => (props.categories ?? []).slice(categoryStart.value, categoryStart.value + categoryPageSize));
watch([() => props.inventoryType, () => props.categories], () => {
...
const pie = computed(() => {
    let angle = 0;
    if (!overviewTotal.value) return 'conic-gradient(#e5efe2 0 360deg)';
    return `conic-gradient(${overviewStatuses.value.map((item) => { const start = angle; angle += (item.value / overviewTotal.value) * 360; return `${item.color} ${start}deg ${angle}deg`; }).join(',')})`;
});
</script>
    ...
    <AuthenticatedLayout>
        <section class="dashboard-shell space-y-6">
            <nav class="inline-flex flex-wrap rounded-2xl border border-[#d8e7d4] bg-white p-1.5 shadow-[0_8px_28px_rgba(39,89,45,.06)]" aria-label="Miri dashboard view">
                <button type="button" class="rounded-xl px-5 py-2.5 text-sm font-bold transition" :class="activeDashboard === 'major' ? 'bg-[#234222] text-white shadow-sm' : 'text-[#60745d] hover:bg-[#f1f7ef] hover:text-[#234222]'" :aria-pressed="activeDashboard === 'major'" @click="router.get(route('major-equipment.dashboard'), { view: 'major', company: companyFilter, inventory_type: inventoryType || 'all' }, { preserveScroll: true })">Major Equipment</button>
                <button type="button" class="rounded-xl px-5 py-2.5 text-sm font-bold transition" :class="activeDashboard === 'rentals' ? 'bg-[#234222] text-white shadow-sm' : 'text-[#60745d] hover:bg-[#f1f7ef] hover:text-[#234222]'" :aria-pressed="activeDashboard === 'rentals'" @click="router.get(route('major-equipment.dashboard'), { view: 'rentals', company: companyFilter, inventory_type: inventoryType || 'all' }, { preserveScroll: true })">Rentals</button>
                <button type="button" class="rounded-xl px-5 py-2.5 text-sm font-bold transition" :class="activeDashboard === 'construction' ? 'bg-[#234222] text-white shadow-sm' : 'text-[#60745d] hover:bg-[#f1f7ef] hover:text-[#234222]'" :aria-pressed="activeDashboard === 'construction'" title="Construction TEC, Garnet & PPE" @click="router.get(route('major-equipment.dashboard'), { view: 'construction', company: companyFilter }, { preserveScroll: true })">TEC, Garnet &amp; PPE</button>
                <button type="button" class="rounded-xl px-5 py-2.5 text-sm font-bold transition" :class="activeDashboard === 'paint' ? 'bg-[#234222] text-white shadow-sm' : 'text-[#60745d] hover:bg-[#f1f7ef] hover:text-[#234222]'" :aria-pressed="activeDashboard === 'paint'" @click="router.get(route('major-equipment.dashboard'), { view: 'paint', company: companyFilter }, { preserveScroll: true })">Paint</button>
            </nav>
            <div class="max-w-sm rounded-2xl border bg-white p-4"><CompanyField v-model="selectedCompany" filter @update:model-value="applyCompany" /></div>
            <MiriPaintDashboard v-if="activeDashboard === 'paint'" :dashboard="paintDashboard" :can-edit="canEditPaint" />
            <MiriConstructionDashboard v-else-if="activeDashboard === 'construction'" :dashboard="constructionDashboard" :can-edit="canEditConstruction" />
            ...
            <nav class="flex flex-wrap gap-2" aria-label="Major Equipment dashboard filter">
                <Link v-for="[key,label] in [['all','All Major Equipment'],['machinery','Machinery'],['cargo','Cargo']]" :key="key"
                    :href="route('major-equipment.dashboard', { inventory_type: key, view: 'major', company: companyFilter })"
            ...
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-12 lg:gap-5">
                <article class="self-start rounded-[1.5rem] border border-[#d9e8d5] bg-white p-5 shadow-[0_12px_35px_rgba(39,89,45,.07)] sm:p-6 lg:col-span-5"><div class="flex items-start justify-between gap-4"><div><p class="text-[10px] font-extrabold uppercase tracking-[.22em] text-blue-500">Equipment allocation</p><h2 class="mt-1 text-xl font-black tracking-tight text-slate-800">Status overview</h2></div><span class="text-right text-xs text-slate-400"><strong class="block text-lg text-slate-700">{{ overviewTotal }}</strong>records</span></div>
                    ...
                    <nav v-if="statusPageCount > 1" class="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-[#e8f0e4] pt-4" aria-label="Status overview pagination">
                        <p class="text-xs text-slate-500" aria-live="polite">Showing {{ statusStart + 1 }}–{{ statusStart + visibleStatuses.length }} of {{ overviewStatuses.length }} statuses · Page {{ statusPage }} of {{ statusPageCount }}</p>
                        <div class="flex gap-2">
                            <button type="button" :disabled="statusPage === 1" @click="statusPage = Math.max(1, statusPage - 1)" class="rounded-lg border border-[#d8e7d4] px-3 py-2 text-sm font-semibold text-[#234222] hover:bg-[#f1f7ef] disabled:cursor-not-allowed disabled:opacity-40">Previous</button>
                            <button type="button" :disabled="statusPage === statusPageCount" @click="statusPage = Math.min(statusPageCount, statusPage + 1)" class="rounded-lg border border-[#d8e7d4] px-3 py-2 text-sm font-semibold text-[#234222] hover:bg-[#f1f7ef] disabled:cursor-not-allowed disabled:opacity-40">Next</button>
                        </div>
                    </nav>
                </article>
                <article class="rounded-[1.5rem] border border-[#d9e8d5] bg-white p-5 shadow-[0_12px_35px_rgba(39,89,45,.07)] sm:p-6 lg:col-span-7"><div class="flex items-start justify-between gap-4"><div><p class="text-[10px] font-extrabold uppercase tracking-[.22em] text-indigo-500">Inventory mix</p><h2 class="mt-1 text-xl font-black tracking-tight text-slate-800">Records by subcategory</h2></div><p class="text-right text-xs text-slate-400"><strong class="block text-lg text-slate-700">{{ categories.length }}</strong>subcategories</p></div>
                    <p class="mt-3 text-xs text-slate-500">Each bar shows status counts within a subcategory. Percentages use that subcategory's total records.</p>
                    <div class="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-xs text-slate-600" aria-label="Status colours">
                        <span v-for="item in statuses" :key="item.label" class="inline-flex items-center gap-2"><span class="h-2.5 w-2.5 rounded-full" :style="{ backgroundColor: item.color }" />{{ statusLabel(item.label) }}</span>
                    </div>
                    <div class="mt-5 space-y-5">
                        <div v-for="item in visibleCategories" :key="item.category" class="rounded-xl p-2">
                            <div class="mb-2 flex items-end justify-between gap-4"><span class="break-words text-sm font-bold text-[#50694e]">{{ item.category }}</span><strong class="shrink-0 text-sm text-[#173a21]">{{ item.total }} records</strong></div>
                            <div class="h-3 overflow-hidden rounded-full bg-slate-100" aria-hidden="true">
                                <div class="flex h-full overflow-hidden rounded-full" :style="{ width: `${(item.total / maximum(categories)) * 100}%` }">
                                    <span v-for="status in groupStatuses(item.statuses)" :key="status.label" class="h-full" :style="{ width: `${(status.value / item.total) * 100}%`, backgroundColor: status.color }" />
                                </div>
                            </div>
                            <div class="mt-3 grid gap-2 text-xs sm:grid-cols-2">
                                <div v-for="status in groupStatuses(item.statuses)" :key="status.label" class="flex items-center gap-2">
                                    <span class="h-2 w-2 shrink-0 rounded-full" :style="{ backgroundColor: status.color }" />
                                    <span class="text-slate-600">{{ statusLabel(status.label) }}:</span>
                                    <strong class="text-slate-800">{{ status.value }} ({{ percent(status.value, item.total) }}%)</strong>
                                </div>
                            </div>
                        </div>
                        <p v-if="categories.length === 0" class="text-sm text-slate-500">No records imported yet.</p>
                    </div>
                    <nav v-if="categoryPageCount > 1" class="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-[#e8f0e4] pt-4" aria-label="Subcategory pagination">
                        <p class="text-xs text-slate-500" aria-live="polite">Showing {{ categoryStart + 1 }}–{{ categoryStart + visibleCategories.length }} of {{ categories.length }} · Page {{ categoryPage }} of {{ categoryPageCount }}</p>
                        <div class="flex gap-2">
                            <button type="button" :disabled="categoryPage === 1" @click="categoryPage = Math.max(1, categoryPage - 1)" class="rounded-lg border border-[#d8e7d4] px-3 py-2 text-sm font-semibold text-[#234222] hover:bg-[#f1f7ef] disabled:cursor-not-allowed disabled:opacity-40">Previous</button>
                            <button type="button" :disabled="categoryPage === categoryPageCount" @click="categoryPage = Math.min(categoryPageCount, categoryPage + 1)" class="rounded-lg border border-[#d8e7d4] px-3 py-2 text-sm font-semibold text-[#234222] hover:bg-[#f1f7ef] disabled:cursor-not-allowed disabled:opacity-40">Next</button>
                        </div>
                    </nav>
                </article>
