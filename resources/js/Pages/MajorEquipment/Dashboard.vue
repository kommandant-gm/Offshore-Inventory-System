...
const statusPageCount = computed(() => Math.max(1, Math.ceil(statuses.value.length / statusPageSize)));
const statusStart = computed(() => (statusPage.value - 1) * statusPageSize);
const visibleStatuses = computed(() => statuses.value.slice(statusStart.value, statusStart.value + statusPageSize));
watch([() => props.inventoryType, () => props.statusBreakdown], () => {
    statusPage.value = 1;
});
            ...
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-12 lg:gap-5">
                <article class="self-start rounded-[1.5rem] border border-[#d9e8d5] bg-white p-5 shadow-[0_12px_35px_rgba(39,89,45,.07)] sm:p-6 lg:col-span-5"><div class="flex items-start justify-between gap-4"><div><p class="text-[10px] font-extrabold uppercase tracking-[.22em] text-blue-500">Equipment allocation</p><h2 class="mt-1 text-xl font-black tracking-tight text-slate-800">Status overview</h2></div><span class="text-right text-xs text-slate-400"><strong class="block text-lg text-slate-700">{{ total }}</strong>records</span></div><div class="mt-7 grid items-center gap-7 sm:grid-cols-[12rem,minmax(0,1fr)]"><div class="relative mx-auto h-44 w-44 rounded-full p-3 shadow-inner sm:h-48 sm:w-48" :style="{ background: pie }"><div class="flex h-full w-full flex-col items-center justify-center rounded-full bg-white shadow-[inset_0_0_0_1px_#edf4ea]"><span class="text-3xl font-black text-[#173a21]">{{ total }}</span><span class="mt-1 text-xs font-semibold text-[#7b8f78]">Total records</span></div></div><div class="min-w-0 space-y-2"><div v-for="item in visibleStatuses" :key="item.label" class="flex items-center gap-3 rounded-xl px-3 py-2.5"><span class="h-2.5 w-2.5 shrink-0 rounded-full ring-4 ring-[#edf5ea]" :style="{ backgroundColor: item.color }"/><span class="min-w-0 flex-1 break-words text-sm font-semibold text-[#5c715a]">{{ item.label }}</span><span class="shrink-0 text-right"><strong class="block text-sm font-black text-[#173a21]">{{ item.value }}</strong><small class="text-[10px] font-bold text-[#93a391]">{{ percent(item.value) }}%</small></span></div><p v-if="statuses.length === 0" class="text-sm text-slate-500">No status records available.</p></div></div>
                    <nav v-if="statusPageCount > 1" class="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-[#e8f0e4] pt-4" aria-label="Status overview pagination">
                        <p class="text-xs text-slate-500" aria-live="polite">Showing {{ statusStart + 1 }}–{{ statusStart + visibleStatuses.length }} of {{ statuses.length }} statuses · Page {{ statusPage }} of {{ statusPageCount }}</p>
                ...
                <article class="rounded-[1.5rem] border border-[#d9e8d5] bg-white p-5 shadow-[0_12px_35px_rgba(39,89,45,.07)] sm:p-6 lg:col-span-7"><div class="flex items-start justify-between gap-4"><div><p class="text-[10px] font-extrabold uppercase tracking-[.22em] text-indigo-500">Inventory mix</p><h2 class="mt-1 text-xl font-black tracking-tight text-slate-800">{{ inventoryType === 'all' ? 'Machinery and Cargo' : 'Records by subcategory' }}</h2></div><p class="text-right text-xs text-slate-400"><strong class="block text-lg text-slate-700">{{ categories.length }}</strong>{{ inventoryType === 'all' ? 'types' : 'subcategories' }}</p></div><div class="mt-7 space-y-5"><div v-for="item in visibleCategories" :key="item.category" class="rounded-xl p-2"><div class="mb-2 flex items-end justify-between gap-4"><span class="truncate text-sm font-bold text-[#50694e]">{{ item.category || 'Uncategorised' }}</span><span class="shrink-0 text-sm"><strong class="text-[#173a21]">{{ item.total }}</strong><small class="ml-1 text-[#91a08f]">({{ percent(item.total) }}%)</small></span></div><div class="h-3 overflow-hidden rounded-full bg-slate-100"><div class="h-full rounded-full bg-[linear-gradient(90deg,#2563eb,#8b5cf6)]" :style="{ width: `${(item.total / maximum(categories)) * 100}%` }"/></div></div><p v-if="categories.length === 0" class="text-sm text-slate-500">No records imported yet.</p></div>
