...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="mt-5 flex gap-3"><Link v-if="$page.props.auth?.user?.can?.assets_edit" class="btn bg-[#4f9f4a] text-white" :href="route('major-equipment.edit', equipment.id)">Edit equipment</Link><Link class="btn" :href="route('major-equipment.index')">Back to Miri Inventory</Link></div>
    </AuthenticatedLayout>
</template>
