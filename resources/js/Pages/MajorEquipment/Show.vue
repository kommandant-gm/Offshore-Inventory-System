...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="grid gap-6 lg:grid-cols-2">
            ...
            <section class="rounded-[2rem] border border-[#d8e7d4] bg-white p-6 shadow-sm"><h2 class="text-lg font-semibold">COG and Certificates</h2><dl class="mt-4 space-y-3 text-sm"><div><dt class="text-slate-500">Issue-out COG</dt><dd>{{ equipment.issue_out_cog_no || '-' }} / {{ equipment.issue_out_cog_date || '-' }}</dd></div><div><dt class="text-slate-500">Received-backload COG</dt><dd>{{ equipment.received_backload_cog_no || '-' }} / {{ equipment.received_backload_cog_date || '-' }}</dd></div></dl><div class="mt-5 space-y-3"><div v-for="certificate in equipment.certificates" :key="certificate.id" class="rounded-xl border p-3"><p class="font-semibold">{{ certificate.certificate_type }}</p><p class="text-sm text-slate-600">{{ certificate.certificate_no || '-' }} · Expiry: {{ certificate.expiry_date || '-' }}</p></div><p v-if="equipment.certificates.length === 0" class="text-sm text-slate-500">No certificates recorded.</p></div></section>
        </div>
