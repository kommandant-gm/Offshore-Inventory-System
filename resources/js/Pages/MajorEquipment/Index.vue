...
const props = defineProps({ equipment: Object, search: String });
...
<template>
    ...
    <AuthenticatedLayout>
        <PageHeader title="Miri Inventory" description="Miri inventory register. The current import is under the Major Equipment category.">
            <Link class="btn bg-[#4f9f4a] text-white" :href="route('major-equipment.import')">Import CSV</Link>
        </PageHeader>
            ...
            <div class="overflow-x-auto">
                <table class="table w-full">
                    ...
                    <tbody><tr v-for="item in equipment.data" :key="item.id">
                        ...
                        <td><Link class="btn btn-sm" :href="route('major-equipment.show', item.id)">View</Link></td>
                    </tr></tbody>
                </table>
            </div>
