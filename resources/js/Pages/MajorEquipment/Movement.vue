...
const props = defineProps({ summary: Object, locations: Array, records: Object, filters: Object, locationOptions: Array, statusOptions: Array });
const search = ref(props.filters.search || '');
const location = ref(props.filters.location || '');
const type = ref(props.filters.type || 'All');
const status = ref(props.filters.status || 'All');
const currentPage = computed(() => props.records.current_page);
const pageSize = ref(Number(props.filters.per_page || 10));
const visit = (page = 1) => router.get(route('major-equipment.movement'), {
    search: search.value, location: location.value, type: type.value, status: status.value,
    per_page: pageSize.value, page,
}, { preserveState: true, preserveScroll: true, replace: true });
let searchTimer;
watch(search, () => { clearTimeout(searchTimer); searchTimer = setTimeout(() => visit(), 300); });
watch([location, type, status, pageSize], () => { clearTimeout(searchTimer); visit(); });
...
const totalPages = computed(() => props.records.last_page);
const paginatedRecords = computed(() => props.records.data);
