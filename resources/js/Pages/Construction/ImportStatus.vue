...
<template>
    <Head title="Construction import progress" />
    <AuthenticatedLayout>
        <section class="space-y-5 rounded-3xl border border-[#d8e7d4] bg-white p-7">
            <h1 class="text-2xl font-bold text-[#234222]">Construction import progress</h1><p>{{ task.filename }}</p>
            ...
            <p v-if="task.result" class="text-sm">{{ task.result.created }} source rows imported; {{ task.result.needs_review }} flagged for detail/grouping review. Duplicate tags are also highlighted in the register. No rows were merged and no historical stock movements were replayed.</p>
            <div class="flex gap-3"><Link :href="route('construction.index')" class="btn">Open register</Link><Link :href="route('construction.import')" class="btn">Import page</Link></div>
        </section>
    </AuthenticatedLayout>
</template>
