...
const preview = () => form.transform(data => ({ ...data, preview: 1 })).get(route('miri-reports.bintulu-paint'), { preserveState: true, preserveScroll: true });
const display = value => value === null || value === '' ? '—' : typeof value === 'number' ? value.toLocaleString(undefined, { maximumFractionDigits: 3 }) : value;
</script>

<template>
    <Head title="Bintulu Yard Paint Inventory Report" />
    <AuthenticatedLayout>
        <div class="space-y-6">
            <div>
                <Link :href="route('miri-reports.index')" class="text-sm text-green-700 hover:underline">Back to Reports</Link>
                <h1 class="mt-2 text-2xl font-bold text-[#234222]">Bintulu Yard Paint Inventory Report</h1>
            ...
            <form class="rounded-2xl border border-[#d8e7d4] bg-white p-5" @submit.prevent="preview">
                <div class="grid gap-4 md:grid-cols-3">
                    ...
                    <p class="self-center text-sm text-slate-500 md:col-span-2">Bintulu only, including Hempel and IP paint.</p>
                </div>
