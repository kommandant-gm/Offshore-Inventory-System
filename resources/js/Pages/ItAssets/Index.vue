...
const bulkEditUrl = computed(() => `${route('it-assets.bulk-edit')}?${selectedIds.value.map((id) => `asset_ids[]=${encodeURIComponent(id)}`).join('&')}`);
      ...
      <div class="overflow-hidden rounded-[1.7rem] border border-[#d8e7d4] bg-white">
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-[#edf3eb] px-5 py-3 text-sm text-[#60745d]">
          ...
          <div class="flex items-center gap-3">
            <Link v-if="canEdit && selectedCount" :href="bulkEditUrl" class="btn btn-sm bg-[#4f9f4a] font-bold text-white hover:bg-[#3f8d3d]">Bulk edit ({{ selectedCount }})</Link>
