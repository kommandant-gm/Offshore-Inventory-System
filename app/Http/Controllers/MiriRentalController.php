...
class MiriRentalController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $query = MiriRentalItem::query()->when($filters['search'] ?? null, fn ($q, $s) => $q->where(function ($q) use ($s) { foreach (['serial_tag_equipment_no', 'description', 'supplier', 'project_contract', 'current_location', 'issue_out_cog_no', 'received_backload_cog_no', 'return_cog_no'] as $column) $q->orWhere($column, 'like', "%{$s}%"); }))
            ->when($filters['category'] ?? null, fn ($q, $v) => $q->where('category', $v))->when($filters['section_1'] ?? null, fn ($q, $v) => $q->where('section_1', $v))->when($filters['section_2'] ?? null, fn ($q, $v) => $q->where('section_2', $v))->when($filters['location'] ?? null, fn ($q, $v) => $q->where('current_location', $v))->when($filters['supplier'] ?? null, fn ($q, $v) => $q->where('supplier', $v))->when($filters['status'] ?? null, fn ($q, $v) => $q->where('status', $v))->when($filters['due_status'] ?? null, function ($q, $value) use ($today) { $q->whereNotIn('status', ['Received Backload', 'Off Hire', 'Returned to Supplier']); return match ($value) { 'overdue' => $q->where(fn ($q) => $q->whereDate('rental_due_date', '<', $today)->orWhere('status', 'Overdue')), 'due_7' => $q->whereBetween('rental_due_date', [$today, $today->copy()->addDays(7)]), 'due_30' => $q->whereBetween('rental_due_date', [$today->copy()->addDays(8), $today->copy()->addDays(30)]), 'later' => $q->whereDate('rental_due_date', '>', $today->copy()->addDays(30)), 'no_date' => $q->whereNull('rental_due_date'), }; })->when(($filters['missing_details'] ?? null) === 'missing', fn ($q) => $q->where(fn ($q) => $q->whereNull('description')->orWhereNull('supplier')->orWhereNull('current_location')))
            ->orderBy('section_1')->orderBy('section_2')->orderBy('description');
        $all = MiriRentalItem::query(); $active = (clone $all)->whereNotIn('status', ['Received Backload', 'Off Hire', 'Returned to Supplier']); $options = fn (string $column) => MiriRentalItem::query()->whereNotNull($column)->where($column, '<>', '')->distinct()->orderBy($column)->pluck($column)->values();
        return Inertia::render('MiriRental/Index', ['rentals' => $query->paginate(25)->withQueryString(), 'summary' => ['total' => (clone $all)->count(), 'on_hire' => (clone $all)->where('status', 'On Hire')->count(), 'issued' => (clone $all)->where('status', 'Issued')->count(), 'backload' => (clone $all)->where('status', 'Received Backload')->count(), 'overdue' => (clone $active)->where(fn ($q) => $q->whereDate('rental_due_date', '<', $today)->orWhere('status', 'Overdue'))->count(), 'due_7' => (clone $active)->whereBetween('rental_due_date', [$today, $today->copy()->addDays(7)])->count(), 'due_30' => (clone $active)->whereBetween('rental_due_date', [$today->copy()->addDays(8), $today->copy()->addDays(30)])->count(), 'no_due_date' => (clone $active)->whereNull('rental_due_date')->count()], 'filters' => array_merge(['search' => '', 'category' => '', 'section_1' => '', 'section_2' => '', 'location' => '', 'supplier' => '', 'status' => '', 'due_status' => '', 'missing_details' => ''], $filters), 'categoryOptions' => $options('category'), 'section1Options' => $options('section_1'), 'section2Options' => $options('section_2'), 'locationOptions' => $options('current_location'), 'supplierOptions' => $options('supplier'), 'statusOptions' => self::STATUSES, 'canEdit' => $request->user()->canEdit('assets')]);
    }
