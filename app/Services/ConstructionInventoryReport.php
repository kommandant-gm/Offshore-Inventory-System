...
abstract class ConstructionInventoryReport
{
    public const COLUMNS = InventoryReportColumns::COLUMNS;

    abstract protected function matches(MiriConstructionItem $item): bool;

    abstract protected function scopeNote(): string;
    ...
    public function generate(int $branch, string $month): array
    {
        ...
        foreach (['miri_construction_items', 'miri_construction_stock_movements'] as $table) {
            if (! Schema::hasTable($table)) {
                return ['rows' => [], 'notes' => $notes, 'unavailable' => $this->reportLabel().' reporting requires the construction register and stock-history database migrations.'];
            }
        }
