...
class ItAssetImportService
{
    ...
    private function csvRows(string $path): array
    {
        ...
        $headers = array_map(fn ($v) => Str::lower(trim((string) $v)), fgetcsv($handle) ?: []); $rows=[];
    ...
    private function xlsxRows(string $path): array
    {
        ...
        if ($sharedXml) { $xml=simplexml_load_string($sharedXml); foreach ($xml->si ?? [] as $si) $shared[]=trim(implode('', array_map(fn ($node) => (string) $node, $si->xpath('.//t') ?: []))); }
