...
class MiriCidbReportTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_cidb_only_bintulu_monthly_balances_and_template_export(): void
    {
        [$user, $branch] = $this->reader();
        $base = ['branch_id' => $branch, 'category' => 'CONSTRUCTION TEC', 'section_1' => 'CIDB TRAINING', 'description' => '=Training tool & kit', 'current_location' => 'BINTULU NEW YARD', 'unit' => 'PCS', 'stock_balance' => 13, 'unit_price' => 5, 'closing_value' => 65, 'created_at' => '2026-08-01'];
        $item = MiriConstructionItem::create($base);
        foreach ([['section_1' => 'WQT TRAINING'], ['current_location' => 'LABUAN YARD'], ['section_1' => 'NONCIDB'], ['branch_id' => Branch::where('code', 'KL-IT')->value('id')]] as $extra) {
        ...
        $this->get(route('miri-reports.bintulu-cidb', ['preview' => 1, 'month' => '2026-09']))->assertOk()->assertInertia(fn (AssertableInertia $p) => $p
            ->component('MiriReports/BintuluCidb')->has('report.rows', 1)->where('report.rows.0.opening', 10)
        ...
        $response = $this->get(route('miri-reports.cidb.export', ['month' => '2026-09']))->assertOk()->assertDownload('bintulu-yard-cidb-training-item-inventory-report-2026-09.xlsx');
        $path = $response->baseResponse->getFile()->getPathname();
        $zip = new ZipArchive;
        $zip->open($path);
        $template = new ZipArchive;
        $template->open(resource_path('report-templates/cidb-training-inventory.xlsx'));
        try {
            $xml = $zip->getFromName('xl/worksheets/sheet1.xml');
            $this->assertStringContainsString('CIDB TRAINING ITEM STOCK INVENTORY STATUS FOR SEPTEMBER 2026', $xml);
            $this->assertStringContainsString('LOCATION : BINTULU NEW YARD', $xml);
            $this->assertStringContainsString('GRAND TOTAL FOR CIDB TRAINING ITEM', $xml);
            ...
            $this->assertStringContainsString('CIDB 2026', $zip->getFromName('xl/workbook.xml'));
    ...
    {
        [, $branch] = $this->reader();
        MiriConstructionItem::create(['branch_id' => $branch, 'category' => 'CIDB', 'current_location' => 'BTU', 'stock_balance' => 8, 'stock_in_qty' => 200, 'unit_price' => 9, 'unit' => 'PCS', 'created_at' => '2026-07-01']);
        $service = app(CidbInventoryReport::class);
    ...
    {
        [$user] = $this->reader();
        foreach (['miri-reports.bintulu-cidb', 'miri-reports.cidb.export'] as $route) {
        ...
        $this->get(route('miri-reports.bintulu-cidb', ['preview' => 1]))->assertOk()->assertInertia(fn (AssertableInertia $p) => $p->has('report.rows', 0)->where('report.unavailable', 'CIDB reporting requires the construction register and stock-history database migrations.'));
        $this->get(route('miri-reports.cidb.export'))->assertUnprocessable();
