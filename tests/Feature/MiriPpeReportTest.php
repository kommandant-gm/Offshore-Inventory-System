...
class MiriPpeReportTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_bintulu_ppe_monthly_balances_and_template_export(): void
    {
        [$user, $branch] = $this->reader();
        $base = ['branch_id' => $branch, 'category' => ' ppe ', 'section_1' => 'GENERAL', 'description' => '=Training tool & kit', 'current_location' => 'BINTULU NEW YARD', 'unit' => 'PCS', 'stock_balance' => 13, 'unit_price' => 5, 'closing_value' => 65, 'created_at' => '2026-08-01'];
        $item = MiriConstructionItem::create($base);
        foreach ([['category' => 'CONSTRUCTION TEC', 'section_1' => 'CIDB TRAINING'], ['current_location' => 'LABUAN YARD'], ['category' => 'CONSUMABLE', 'section_1' => 'WQT TRAINING'], ['branch_id' => Branch::where('code', 'KL-IT')->value('id')]] as $extra) {
        ...
        $this->get(route('miri-reports.bintulu-ppe', ['preview' => 1, 'month' => '2026-09']))->assertOk()->assertInertia(fn (AssertableInertia $p) => $p
            ->component('MiriReports/BintuluPpe')->has('report.rows', 1)->where('report.rows.0.opening', 10)
        ...
        $response = $this->get(route('miri-reports.ppe.export', ['month' => '2026-09']))->assertOk()->assertDownload('bintulu-yard-ppe-inventory-report-2026-09.xlsx');
        $path = $response->baseResponse->getFile()->getPathname();
        $zip = new ZipArchive;
        $zip->open($path);
        $template = new ZipArchive;
        $template->open(resource_path('report-templates/ppe-inventory.xlsx'));
        try {
            $xml = $zip->getFromName('xl/worksheets/sheet1.xml');
            $this->assertStringContainsString('GENERAL STORE PPE INVENTORY STATUS FOR SEPTEMBER 2026', $xml);
            $this->assertStringContainsString('LOCATION : BINTULU YARD', $xml);
            $this->assertStringContainsString('GRAND TOTAL FOR PPE ITEM', $xml);
            ...
            $this->assertStringContainsString('PPE 2026', $zip->getFromName('xl/workbook.xml'));
    ...
    {
        [, $branch] = $this->reader();
        MiriConstructionItem::create(['branch_id' => $branch, 'category' => 'PPE', 'section_1' => 'WQT TRAINING', 'current_location' => 'BTU', 'stock_balance' => 8, 'stock_in_qty' => 200, 'unit_price' => 9, 'unit' => 'PCS', 'created_at' => '2026-07-01']);
        $service = app(PpeInventoryReport::class);
    ...
    {
        [$user] = $this->reader();
        foreach (['miri-reports.bintulu-ppe', 'miri-reports.ppe.export'] as $route) {
        ...
        $this->get(route('miri-reports.bintulu-ppe', ['preview' => 1]))->assertOk()->assertInertia(fn (AssertableInertia $p) => $p->has('report.rows', 0)->where('report.unavailable', 'PPE reporting requires the construction register and stock-history database migrations.'));
        $this->get(route('miri-reports.ppe.export'))->assertUnprocessable();
